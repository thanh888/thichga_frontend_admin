...
const columns: Column[] = [
  ...
  { id: 'userProfit', label: 'Tiền thắng', minWidth: 120, align: 'left' },
  { id: 'systemProfit', label: 'Hoa hồng', minWidth: 150, align: 'left' },
...
export default function HistoryOptionExGamePage(): React.JSX.Element {
  ...
  const optionID = param?.id?.toString();
  ...
  React.useEffect(() => {
    fetchBets();
  }, [filter, page, rowsPerPage, order, orderBy, optionID]);
            ...
            {bets?.docs?.map((row: BettingHistoryInterface) => (
              <TableRow hover key={row?._id}>
                ...
                <TableCell>
                  {row?.userResult ? (
                    (() => {
                      const result = listResultHistory.find((item) => item.value === row.userResult) as any;
                      if (!result) return null;

                      const color =
                        result.value === BetResultEnum.WIN
                          ? '#4caf50' // xanh lá
                      ...
                      return <Typography sx={{ color, fontWeight: 600 }}>{result.label}</Typography>;
                    })()
                  ) : (
                    <Typography color="text.secondary">{row.status}</Typography>
                  )}
                </TableCell>
                <TableCell>
                  {row?.userResult ? (
                    (() => {
                      ...
                      return <Typography sx={{ color, fontWeight: 600 }}>{row.userProfit}</Typography>;
                    })()
                  ) : (
                ...
                <TableCell>
                  {row?.userResult ? (
                    (() => {
                      ...
                      return <Typography sx={{ color, fontWeight: 600 }}>{row.systemProfit}</Typography>;
                    })()
                  ) : (
                    <Typography color="text.secondary">Đang chờ</Typography>
                  )}
